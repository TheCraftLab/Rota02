...
export function SummaryPanel({ rotation }: SummaryPanelProps) {
  ...
  return (
    <section className="panel-surface layout-safe overflow-hidden rounded-4xl border border-gray-100 p-6 shadow-panel">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-widest text-slate/50">Controle</p>
          <h2 className="mt-2 text-xl font-semibold text-ink">Vision d'ensemble</h2>
        </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <div className="layout-safe rounded-xl border border-gray-100 bg-gray-50 p-4">
          <p className="text-sm font-medium text-ink">Repartition par agent</p>
          <div className="mt-4 max-w-full overflow-x-auto">
            <table className="min-w-[600px] w-full text-left text-sm">
              <thead>
                <tr className="text-xs text-slate/60">
              ...
              <tbody className="text-ink">
                {rotation.summary.agentSummaries.map((summary) => (
                  <tr key={summary.agentName} className="border-t border-gray-100">
                    ...
                    <td className="py-2.5">
                      {summary.overload ? <StatusBadge tone="warning">Surveillance</StatusBadge> : <span className="text-xs text-slate/50">RAS</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="layout-safe rounded-xl border border-gray-100 bg-gray-50 p-4">
          <p className="text-sm font-medium text-ink">Alertes</p>
          <div className="mt-4 flex flex-col gap-2">
            {rotation.summary.alerts.length ? (
              rotation.summary.alerts.map((alert) => (
                <div key={alert} className="rounded-lg border border-coral/15 bg-coral/5 px-4 py-3 text-sm text-coral">
                  {alert}
                </div>
              ))
            ) : (
              <div className="rounded-lg border border-mint/15 bg-mint/5 px-4 py-3 text-sm text-mint">
