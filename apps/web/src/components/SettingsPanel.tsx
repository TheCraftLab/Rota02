...
export function SettingsPanel({ settings, disabled, loading, onGenerate, onSettingsChange }: SettingsPanelProps) {
  const invalidTimeRange = settings.startTime >= settings.endTime;
  ...
  return (
    <section className="panel-surface rounded-4xl border border-gray-100 p-6 shadow-panel">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <button
          type="button"
          disabled={disabled || loading || invalidTimeRange}
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-3">
        <label className="rounded-xl border border-gray-100 bg-gray-50 p-4">
          <span className="text-xs font-medium text-slate/60">Debut</span>
          <input
        ...
        <label className="rounded-xl border border-gray-100 bg-gray-50 p-4">
          <span className="text-xs font-medium text-slate/60">Duree des creneaux</span>
          <select
            className="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-ink outline-none ring-amber/30 transition focus:border-amber/60 focus:ring-4"
      ...
      {invalidTimeRange ? (
        <p className="mt-3 rounded-lg border border-coral/20 bg-coral/8 px-3 py-2 text-sm text-coral">
